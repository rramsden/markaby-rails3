...
module Markaby
  RUBY_VERSION_19 = (RUBY_VERSION =~ /1.9/)
    ...
    def tag!(tag, *args, &block)
      ...
      if @auto_validation && @tagset
        if !@tagset.tagset.has_key?(tag)
          ...
          # this adds a slight performance hit + 0.03ms this is to ensure
          # attributes are displayed in classic Ruby 1.8 sorted order
          args[args.size-1] = Hash[args.last.sort] if RUBY_VERSION_19
          
          attrs = args.last

          if @tagset.forms.include?(tag) && attrs[:id]
            # preservers attribute ordering in 1.9 see above
            attrs[:name] = attrs.delete(:id)
            attrs[:id] = attrs[:name]
          end
  ...
  private
    ...
    def method_missing(sym, *args, &block)
      ...
      elsif @helpers && instance_variables_for(@helpers).include?(ivar)
  ...
  # Markaby::Fragment implements a dynamic interface for dispatching method calls via
  # method_missing. This is a problem if you call any standard Ruby object methods like 
  # inspect, to_s, etc. Ruby 1.9 has a built-in object called BasicObject to handle this. 
  DynamicObject = RUBY_VERSION_19 ? BasicObject : ::Builder::BlankSlate
